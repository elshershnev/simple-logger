    ...
    constructor(options) {
        ...
        if (!fileName) {
            ...
            if (options.rotationParameters) {
                ...
                this._rotationParameters.interval = options.rotationParameters.interval || 86400000;
                this._rotationParameters.compress = options.rotationParameters.compress || false;
                this._rotationParameters.keepLogs = options.rotationParameters.keepLogs || 30;

                this._timerId = setInterval(function(){this.rotate()}.bind(this), this._rotationParameters.interval);
            }
        }
    }
                        ...
                        fs.rename(oldFile, newFile, function(err) {
                            if (err) {
                                ...
                                fs.unlink(newFile, function(err) {
                                    // TODO to think about throwing an
                                    // exception here
                                });
                            }
                            ...
                            if (!task) {
                                ...
                                if (compress) {
                                    target = fs.createWriteStream(path.join(filePath,fileName + '.1.gz'));
                                }
                                else {
                                    target = fs.createWriteStream(path.join(filePath,fileName + '.1'));
                                }
